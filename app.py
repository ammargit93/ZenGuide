...
def get_context(user_name):
    context_entry = context_coll.find_one({"user": user_name})
    if context_entry:
        return context_entry["history"]
    return []


# Helper function to save/update context in MongoDB
def save_context(user_name, context_history):
    context_coll.update_one(
        {"user": user_name},
        {"$set": {"history": context_history, "timestamp": datetime.datetime.now()}},
        upsert=True
    )
...
async def generate_response(user_message: str, user_name: str):
    context_history = get_context(user_name)
    context_history.append(f"User: {user_message}")
...
def format_to_points(text, use_numbers=False):
    ...
    for line in lines:
        ...
        if any(line.startswith(f"{i})") for i in range(1, 10)):  # Check for 1), 2), ... 9)
            ...
            current_point = line
        # If it starts with a bullet or other markers
